...
with col3:
    anxiety = st.selectbox("Do you have Anxiety?",      ["No","Yes"], key="knn_anxiety")
    panic   = st.selectbox("Do you have Panic Attack?", ["No","Yes"], key="knn_panic")
    st.write("")
    st.write("")
    st.write("")
    predict_btn = st.button("Predict Depression Risk", use_container_width=True,
                            key="knn_predict")
...
if predict_btn:
    ...
    st.session_state.knn_result = {
        ...
        'year': year, 'cgpa': cgpa, 'anxiety': anxiety, 'panic': panic,
    }

# ── Show result (persists across rerenders) ────────────────────
    ...
    with r2:
        ...
        summary_df = pd.DataFrame({
            "Field": ["Name","Gender","Age","Course","Year",
                      "CGPA","Anxiety","Panic Attack"],
            "Value": [R['name'], R['gender'], R['age'], R['course'],
                      R['year'], R['cgpa'], R['anxiety'], R['panic']]
        }).set_index("Field")
